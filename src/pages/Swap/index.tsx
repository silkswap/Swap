import { CurrencyAmount, JSBI, Token, TokenAmount, Trade } from '@pancakeswap-libs/sdk'
...
import { INITIAL_ALLOWED_SLIPPAGE, PLAYER_MANAGER } from 'constants/index'
...
import MiningHeader from 'components/a/MiningHeader'
...
import { usePlayerManagerContract } from '../../hooks/useContract'
import useRefresh from '../../hooks/useRefresh'
import { addTransaction } from '../../state/transactions/actions'
import PLAYER_MANAGER_ABI from '../../constants/abis/playermanager.json'

const StyLink = styled(Link)`
  display: inline;
  color: ${({ theme }) => theme.colors.failure};
`
const MiningText = styled.div`
  font-size: 14px;
  color: #333333;
`
...
const Swap = () => {
  ...
  const playerManager = usePlayerManagerContract();
  ...
  useEffect(() => {
    const fetchAllBalances = async () => {
      if(playerManager){
        const result = await playerManager._plyr(account);
        const slk = new Token(56,'0x459752cb1e058cfa9ee6d1bc4f60a64012f94d1e',18,'SLK','SLK');
        const datas = new TokenAmount(slk, result[1]);
        setReward(datas.toFixed(6));
      }
    }
    if (account) {
      fetchAllBalances()
    }
  }, [account,playerManager,fastRefresh,setReward])

  const getReward = useCallback(async ()=>{
    if(playerManager){
      await playerManager.functions.getReward();
    }
  },[playerManager]);
  ...
  return (
    <Container>
      ...
      {/* <CardNav /> */}
      <AppBody>
        <MiningHeader title={intl.get('Swap.Trade_Mining')} />
        <CardBody>
          <MiningText style={{marginBottom: '30px'}}>{intl.get('Swap.Participate_transaction')}</MiningText>
          <MiningText style={{marginBottom: '8px'}}>{intl.get('Swap.Your_commission_rate')}</MiningText>
          <MiningText style={{marginBottom: '30px'}}>0.2 %( {intl.get('Swap.Depends_amount')})</MiningText>
          <div style={{fontSize: '22px', color:'#008d3e',marginBottom: '2px'}}>{reward}</div>
          <RowBetween align="center">
            <div style={{fontSize: '14px', color: '#999999'}}>SLK can be harvest</div>
            <StyBtn variant='primary' onClick={getReward}>Harvest</StyBtn>
          </RowBetween>
        </CardBody>
      </AppBody>
      ...
      <AppBody>
        <Wrapper id="swap-page">
          ...
          />
          <PageHeader
            title={TranslateString(8, intl.get('Swap.Swap_text'))}
          />
          <CardBody>
            <AutoColumn gap="md">
              <CurrencyInputPanel
                label={
                  independentField === Field.OUTPUT && !showWrap && trade
                    ? TranslateString(194, intl.get('Swap.From_estimated'))
                    : TranslateString(76, intl.get('Swap.From'))
                }
              ...
              <CurrencyInputPanel
                ...
                label={
                  independentField === Field.INPUT && !showWrap && trade
                    ? TranslateString(196,intl.get('Swap.To_estimated'))
                    : TranslateString(80, intl.get('Swap.To'))
                }
                showMaxButton={false}
                ...
                <Card padding=".25rem .75rem 0 .75rem" borderRadius="20px">
                  <AutoColumn gap="4px">
                    ...
                    {allowedSlippage !== INITIAL_ALLOWED_SLIPPAGE && (
                      <RowBetween align="center">
                        <Text fontSize="14px">{TranslateString(88, intl.get('Swap.Slippage_Tolerance'))}</Text>
            ...
            <BottomGrouping>
              ...
              {!account ? (
                ...
                <GreyCard style={{ textAlign: 'center' }}>
                  <Text mb="4px">{TranslateString(1194, intl.get('Swap.Insufficient_liquidity'))}</Text>
                </GreyCard>
              ) : showApproveFlow ? (
                <RowBetween>
                  <Button
