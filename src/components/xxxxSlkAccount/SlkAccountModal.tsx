    ...
    <Wrap style={{zIndex: 100}}>
      ...
      <Card>
        <RowBetween mb="16px" mt="30px">
          <Label>Your SLK</Label>
          <Label>Can be harvest</Label>
        </RowBetween>
        <RowBetween>
          <NumText>100.000000</NumText>
          <Row>
            <NumText>0.000000</NumText>
            <Btn onClick={onPresentSlkAccount} width="75px" ml="12px">Harvest</Btn>
          </Row>
        </RowBetween>

        <Line />
        <RowBetween mb="22px">
          <Label>SLK price:</Label>
          <Val>$ 1.12</Val>
        </RowBetween>
        <RowBetween mb="22px">
          <Label>SLK in circulation:</Label>
...
const SlkAccountModal = ({ onDismiss = defaultOnDismiss }: SlkAccountModalProps) => {
  return (
    <div>hhhhhhhh</div>
  )
}
